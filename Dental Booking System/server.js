...
app.post('/signup', (req, res) => {
    ...
    if (userExists) {
        res.status(401).json({ success: false, message: 'Username or phone number already exists' });
    } else {
...
app.post('/book', (req, res) => {
    ...
    if (!isFutureDate(date, startTime)) {
        res.status(409).json({ success: false, message: 'Cannot book an appointment in the past' });
        return;
    }
    if (!isValidDuration(startTime, endTime)) {
        res.status(400).json({ success: false, message: 'Appointment must be at least 20 minutes' });
        return;
    }
...
app.post('/cancel', (req, res) => {
    ...
    if (appointmentIndex !== -1) {
        ...
        res.status(401).json({ success: false, message: 'Appointment canceled successfully' });
    }
});
